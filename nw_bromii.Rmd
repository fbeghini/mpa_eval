...
```{bash}
cat /shares/CIBIO-Storage/CM/news/users/leonard.dubois/rbromii4000_panphlan/cluster1.tree | tr ',' '\n' | tr -d '('| tr -d ')' | tr -d "'" | cut -f1 -d ':' > /tmp/cluster2
cat /shares/CIBIO-Storage/CM/news/users/leonard.dubois/rbromii4000_panphlan/cluster2.tree | tr ',' '\n' | tr -d '('| tr -d ')' | tr -d "'" | cut -f1 -d ':' > /tmp/cluster1
```



```{r}
panphlan_table <- fread('/shares/CIBIO-Storage/CM/news/users/leonard.dubois/rbromii4000_panphlan/profile_4000_rbromii.csv')
metadata <- fread('/shares/CIBIO-Storage/CM/news/users/leonard.dubois/rbromii4000_panphlan/metadata_eu.csv')
...
## To NR50
```{r}
# rbromii_annot <- fread('/shares/CIBIO-Storage/CM/news/users/francesco.beghini/hg/chocophlan/export_201901/humann2/HUMAnN_3.0_CHOCOPhlAn_201901_functional_annotation_mapping.tsv', sep = '\t', nThread = 12)
# c <- fread('/shares/CIBIO-Storage/CM/news/users/leonard.dubois/PanPhlAn_work/old_pangenomes/panphlan_rbromii_pangenome.csv', nThread = 12)
...
```{r}
NR50names <- fread('/shares/CIBIO-Storage/CM/news/users/francesco.beghini/chocophlan_paper/cluster2_enrich_names', col.names = c('NR50','descr'))
